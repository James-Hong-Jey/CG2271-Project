...
{
	// Clear Pending IRQ
	NCIV_ClearPendingIRQ(PORTD_IRQn); // Not necessary, included for legacy reasons
	...
	// From manual: Set a 1 to it in order to reset it
	PORTD->ISFR = 0xffffffff; // If not it will trigger indefinitely
}
...
void initSwitch(void) {
	...
	// Select GPIO, enable pullups, interrupts on falling edge
	PORTD->PCR[SW_POS] |= (PORT_PCR_MUX(1) | 
							PORT_PCR_PS_MASK | // PS = Pull Select
							PORT_PCR_PE_MASK | // PE = Pull Enable
							PORT_PCR_IRQC(0x0a));  // 0x0a = 1010 = Interrupt on Falling Edge

	// Set PORT D Switch bit to input
	PTD->PDDR &= ~MASK(SW_POS); // Datasheet pg778, clear SW_POS

	// Enable Interrupts
	NVIC_SetPriority(PORTD_IRQn, 2); // For this lab only one interrupt, so 0 1 2 or 3 doesnt matter
