...
/*void led_control(led_colours_t colour) {
  if (colour == RED) {
    PTB->PCOR = MASK(RED_LED); //turn on red
    PTB->PSOR = MASK(GREEN_LED); //turn off green
    PTD->PSOR = MASK(BLUE_LED); //turn off blue
    delay(0x80000); //default clock based on lab2 worksheet
  } else if (colour == GREEN) {
    PTB->PSOR = MASK(RED_LED); //turn off red
    PTB->PCOR = MASK(GREEN_LED); //turn on green
    PTD->PSOR = MASK(BLUE_LED); //turn off blue
    delay(0x80000);
  } else { //if blue
    PTB->PSOR = MASK(RED_LED); //turn off red
    PTB->PSOR = MASK(GREEN_LED); //turn off green
    PTD->PCOR = MASK(BLUE_LED); //turn on blue
    delay(0x80000);
  }
} */
